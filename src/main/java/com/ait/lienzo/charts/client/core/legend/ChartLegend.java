...
public class ChartLegend extends Group {
	private static final int LEGEND_RECTANGLE_WIDTH = 30;

	private static final int LEGEND_RECTANGLE_HEIGHT = 30;

	private static final int LEGEND_HEIGHT_SEPARATION = 20;

	private static final int LEGEND_WIDTH_SEPARATION = 20;
	...
	private ChartLegend buildVertical() {
		...
		for (int x = 0; x < entries.size(); x++) {
			...
			final Rectangle rectangle = new Rectangle(LEGEND_RECTANGLE_WIDTH, LEGEND_RECTANGLE_HEIGHT).setX(legendX).setY(legendY)
					.setFillColor(serie.getColor());
			final Text text = new Text(serie.getName(), FONT_FAMILY, FONT_STYLE, FONT_SIZE).setFillColor(ColorName.BLACK)
					.setX(legendX + LEGEND_RECTANGLE_WIDTH + 10).setY(legendY + (LEGEND_HEIGHT_SEPARATION / 2)).setTextBaseLine(TextBaseLine.MIDDLE);
			add(rectangle);
			add(text);
		}
		return this;
	...
	private ChartLegend buildHorizontal() {
		...
		for (int x = 0; x < size; x++) {
			...
			final Rectangle rectangle = new Rectangle(LEGEND_RECTANGLE_WIDTH, LEGEND_RECTANGLE_HEIGHT).setX(legendX).setY(legendY)
					.setFillColor(serie.getColor());
			final Text text = new Text(serie.getName(), FONT_FAMILY, FONT_STYLE, FONT_SIZE).setFillColor(ColorName.BLACK)
					.setX(legendX + LEGEND_RECTANGLE_WIDTH).setY(legendY + LEGEND_RECTANGLE_HEIGHT / 2).setTextBaseLine(TextBaseLine.MIDDLE);
			add(rectangle);
			add(text);
			if (x == 0)
				firstText = text;
